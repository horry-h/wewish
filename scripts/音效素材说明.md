...
### 1. 翻书音效 (page-flip.wav)
- **位置**: `/program/miniprogram/assets/audio/page-flip.wav`
- **状态**: ✅ 已添加
- **时长**: 约 0.1-0.2秒
- **音量**: 中等偏小
- **描述**: 清脆的书页翻动声
...
### 2. 海浪音效 (wave.mp3) - 推荐
- **位置**: `/program/miniprogram/assets/audio/wave.mp3`
- **状态**: ⏳ 待添加（可选但强烈推荐）
- **时长**: 1.5秒左右的单次海浪声
- **音量**: 很小（背景音）
- **描述**: 轻柔的海浪拍打声，配合震动呼吸效果
- **循环**: 否（代码中会自动循环调用）
- **作用**: 与震动配合实现"通感"设计，提升沉浸感

### 3. 背景氛围音效 (ambient.mp3) - 可选
...
### 海浪呼吸式震动模式
当前版本已实现高级的海浪呼吸震动，模拟真实海浪的节奏：

**震动序列**（单次海浪，约3秒，柔和模式）：
```
浪起 (800ms):  light (400ms) → light (400ms)
浪峰 (900ms):  light (400ms) → medium (500ms)  ← 最强点（降低振幅）
浪落 (700ms):  light (400ms) → light (300ms)
余波 (600ms):  light (600ms)
平静期: 400ms
→ 循环下一波海浪（每3.4秒一轮）
```
...
**优化特点**：
- 🌊 减少震动频率：从13次降至7次震动
- 💫 降低震动振幅：去除 heavy，最强仅用 medium
- 🕊️ 增加间隔时间：每次震动间隔300-600ms
- 💆 更加柔和舒适，不会让人感到刺激
- 📱 iOS 设备体验最佳（Taptic Engine）
- 🤖 Android 自动适配不同机型
