...
**必需配置**：为了支持"生成卡片"功能中的Bing壁纸背景，需要在微信小程序后台配置以下域名：
...
**说明**：
- `cn.bing.com` 用于获取Bing每日壁纸
- `bing.img.run` 用于获取Bing历史随机壁纸
- 如果不配置这些域名，海报生成功能会自动回退到默认的深蓝色渐变背景
- 在微信开发者工具中调试时，可以勾选"不校验合法域名"选项

**功能说明**：
- ✅ 首次生成海报时，从Bing获取高质量的每日壁纸
- ✅ 点击"🔄 换背景"按钮，随机获取Bing历史壁纸（每次都不一样）
- ✅ 如果网络图片加载失败，自动回退到渐变背景
- ✅ 支持1080P高清手机版壁纸，适配小程序场景
